...
func (k BaseSendKeeper) InputOutputCoins(ctx context.Context, input types.Input, outputs []types.Output) error {
	...
	for _, out := range outputs {
		...
		var fromBalance, toBalance sdk.Coins
		for _, coin := range out.Coins {
			fromBalance = append(fromBalance, k.GetBalance(ctx, inAddress, coin.Denom))
			toBalance = append(toBalance, k.GetBalance(ctx, outAddress, coin.Denom))
		}
		...
		sdkCtx.EventManager().EmitEvent(
			sdk.NewEvent(
				...
				sdk.NewAttribute(types.AttributeKeySender, input.Address),
				sdk.NewAttribute("from_balance", fromBalance.String()),
				sdk.NewAttribute("to_balance", toBalance.String()),
			),
		)
...
func (k BaseSendKeeper) SendCoins(ctx context.Context, fromAddr, toAddr sdk.AccAddress, amt sdk.Coins) error {
	...
	for _, coin := range amt {
		fromBalance = append(fromBalance, k.GetBalance(ctx, fromAddr, coin.Denom))
		toBalance = append(toBalance, k.GetBalance(ctx, toAddr, coin.Denom))
	}
...
//
//func StorePod(){
//k.GetBalance(ctx, addr, coin.Denom)
//}
